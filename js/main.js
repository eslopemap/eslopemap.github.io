import {
  haversineKm, normalizeTileX, lonLatToTile, tileLatCenter,
  tileToLngLatBounds, mercatorVertsForTile,
  decodeTerrarium, encodeTerrarium,
  cssColorToRgb01, downloadFile, parseBooleanParam,
} from './utils.js';
...
const DEM_MAX_Z = 14;
const SLOPE_RELIEF_CROSSFADE_Z = 11;
const CORE_DIM = 512;
...
function showCursorTooltipAt(screenX, screenY, eleText, slopeText) {
...
function updateCursorInfoVisibility() {
...
function updateLegend() {
  ...
  // In slope+relief mode, show the legend for whichever sub-mode is active at the current zoom
...
function computeEffectiveSlopeOpacity(map) {
...
function applyModeState(map) {
  computeEffectiveSlopeOpacity(map);
  if (map.getLayer('dem-color-relief')) {
    if (state.mode === 'slope+relief') {
      // Show color-relief below crossfade zoom, fade out over one zoom level
...
map.on('load', () => {
  ...
  map.on('zoomend', () => {
    syncViewToUrl(map);
    if (state.showTileGrid) updateDebugGridSource(map);
    if (state.mode === 'slope+relief') updateLegend();
  });
  map.on('rotateend', () => {
    syncViewToUrl(map);
  });
  ...
  const updateCursorElevation = () => {
    ...
    if (!result) {
      ...
      if (lastPointerScreenXY) showCursorTooltipAt(lastPointerScreenXY.x, lastPointerScreenXY.y, 'no tile', 'n/a');
      return;
    }

    const eleText = `${result.elevation.toFixed(0)} m`;
    const slopeStr = result.slopeDeg != null ? `${result.slopeDeg.toFixed(0)}°` : 'n/a';
    setCursorInfo(eleText, slopeStr);
    if (lastPointerScreenXY) showCursorTooltipAt(lastPointerScreenXY.x, lastPointerScreenXY.y, eleText, slopeStr);
  };
    ...
    map.on('click', (e) => {
      ...
      if (state.cursorInfoMode !== 'no') {
        ...
        if (state.cursorInfoMode === 'cursor' && result) {
          const eleText = `${result.elevation.toFixed(0)} m`;
          const slopeStr = result.slopeDeg != null ? `${result.slopeDeg.toFixed(0)}°` : 'n/a';
          showCursorTooltipAt(e.originalEvent.clientX, e.originalEvent.clientY, eleText, slopeStr);
        }
      }
    });
    map.on('dragstart', () => {
...
let editingIsNewTrack = false; // true when track was just created via draw button
let dragVertexInfo = null; // {trackId, index} — desktop drag
let mobileSelectedVertex = null; // {trackId, index} — mobile move mode
let suppressNextMapClick = false;
let hoverInsertInfo = null; // {lngLat, insertAfter} — desktop hover-insert marker
let selectedVertexIndex = null; // index of selected vertex in editing track
let insertAfterIdx = null; // when set, new points insert after this index
const isLocalhost = location.hostname === 'localhost' || location.hostname === '127.0.0.1';
let mobileFriendlyMode = isMobile; // default on for mobile
let insertPopupMarker = null; // MapLibre marker for on-map + button
...
function syncUndoBtn() {
  ...
  // Show mobile-friendly button on mobile when editing, or on localhost for debug
...
// --- On-map insert popup (+ button near selected vertex) ---
...
function ensureProfileHoverLayer() {
  ...
  // Hover-insert marker source/layer
  ...
  // Insert-preview line source/layer (dashed line showing where next point will connect)
...
// --- Insert preview: show dashed line from attachment points to cursor/crosshair ---
let insertPreviewLngLat = null; // updated by mousemove or mobile crosshair
function updateInsertPreview() {
  ...
  // Determine the target point (where the next click would go)
  ...
  if (insertAfterIdx != null && insertAfterIdx < t.coords.length) {
    // Insert mode: show lines from insertAfterIdx to target and from target to insertAfterIdx+1
    ...
    // Append mode: show line from last point to target
...
function exitEditMode() {
  ...
  if (wasNewTrack && t && t.coords.length < 2) {
    // Auto-cleanup new tracks with < 2 points
...
// --- Enter/exit editing for new tracks (draw button) ---
...
undoBtn.addEventListener('click', () => {
  ...
  t.coords.pop();
  // Adjust selectedVertexIndex and insertAfterIdx if needed
...
map.on('click', (e) => {
  ...
  if (editingTrackId) {
    ...
    // Check vertex hit for selection
    const hitPt = hitTestVertex(e.point);
    if (hitPt && hitPt.index != null) {
      // Mobile friendly mode: start move
      if (mobileFriendlyMode) {
    ...
    // Deselect mobile move if active
    ...
    if (mobileFriendlyMode) {
      // Insert at center crosshair position
...
// --- Vertex hit test (only hits when track is being edited) ---
...
if (!isMobile) {
  ...
  let dragMoved = false;
  let hoverInsertDrag = false; // true when dragging a just-inserted hover vertex

  function finishVertexDrag() {
    if (!dragVertexInfo) return;
    const t = tracks.find(tr => tr.id === dragVertexInfo.trackId);
    // If barely moved, treat as vertex click (selection)
    if (!dragMoved && t) {
  ...
  map.on('mousedown', (e) => {
    ...
    // Check hover-insert marker hit (if near track line)
    if (hoverInsertInfo) {
  ...
  map.on('mousemove', (e) => {
    ...
    if (editingTrackId) {
      // Update insert preview with current cursor position
      if (!mobileFriendlyMode) {
      ...
      // Smart hover-insert marker: show when near track line but not near vertex
      if (!isRealVertex && !hoveredVertex) {
        const t = tracks.find(tr => tr.id === editingTrackId);
...
if (isMobile) {
  // Long-press drag for non-mobile-friendly mode
  let touchLongPressTimer = null;
  let touchStartPt = null;
  let mobileDragVertex = null; // {trackId, index, screenX, screenY}
  ...
  // Mobile-friendly mode: pan moves selected vertex to center
  map.on('move', () => {
...
function cancelMobileMove() {
  ...
  map.dragPan.enable();
  const t = getActiveTrack();
  if (t) { renderTrackList(); updateProfile(); syncUndoBtn(); }
}
...
function setProfileHoverVertex(index) {
  ...
  // Show cursor tooltip at the vertex's screen position
  if (state.cursorInfoMode === 'cursor') {
    const result = queryLoadedElevationAtLngLat(map, {lng: coord[0], lat: coord[1]});
    if (result) {
      const pt = map.project([coord[0], coord[1]]);
      const rect = map.getCanvas().getBoundingClientRect();
      showCursorTooltipAt(rect.left + pt.x, rect.top + pt.y, `${result.elevation.toFixed(0)} m`, result.slopeDeg != null ? `${result.slopeDeg.toFixed(0)}°` : 'n/a');
    }
  }
}
...
// Expose key variables to global scope for E2E test access.
// Module-scoped vars aren't reachable via (0, eval)('varName'),
// so we define getters on window so the same test helpers work.
Object.defineProperties(window, {
  mapReady:            { get() { return mapReady; } },
  map:                 { get() { return map; } },
  tracks:              { get() { return tracks; } },
  activeTrackId:       { get() { return activeTrackId; } },
  editingTrackId:      { get() { return editingTrackId; } },
  selectedVertexIndex: { get() { return selectedVertexIndex; } },
  insertAfterIdx:      { get() { return insertAfterIdx; } },
  mobileFriendlyMode:  { get() { return mobileFriendlyMode; } },
  importFileContent:   { get() { return importFileContent; } },
  profileChart:        { get() { return profileChart; } },
  profileClosed:       { get() { return profileClosed; } },
});
